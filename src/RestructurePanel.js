...
export default function RestructurePanel({ noteTitle, noteHtml, onClose, onApply }) {
  const noteText = htmlToPlainText(noteHtml);
  const [purpose, setPurpose] = useState(() => inferPurpose(noteText));
  const [suggested] = useState(() => inferPurpose(noteText));
  ...
  async function generate() {
    if (!noteText.trim()) {
      setError('This note has no text content to restructure yet.');
      setPhase('error');
      return;
    }
    ...
    try {
      const raw = await aiComplete({
        system: buildRestructureSystemPrompt(purpose, freeform.trim()),
        content: buildRestructureUserContent(noteTitle, noteText),
        maxTokens: 8192,
  ...
  return (
    <div className="lp-overlay" onPointerDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="lp-panel" role="dialog" aria-label="Restructure note">
        <div className="lp-head">
          <span className="lp-head-icon"><RestructureIcon /></span>
          <span className="lp-head-title">Restructure note</span>
        ...
        {phase === 'setup' && (
          <div className="lp-body">
            <p className="lp-intro">
              Reorganize <strong>{noteTitle || 'this note'}</strong> for a purpose — content stays, structure changes.
              Nothing is overwritten until you apply the preview.
            </p>
            ...
            <div className="lp-foot">
              <button className="lp-btn lp-btn-primary" onPointerDown={generate}>Preview restructure</button>
            </div>
          </div>
        )}
        ...
        {phase === 'preview' && (
          <div className="lp-body">
            <p className="lp-intro">
              Preview — <strong>{purposeLabel}</strong>. Your note is untouched until you apply.
            </p>
            <div
            ...
            <div className="lp-foot">
              ...
              <button className="lp-btn lp-btn-primary" onPointerDown={() => onApply(resultHtml)}>Apply to note</button>
            </div>
          </div>
        )}
      </div>
